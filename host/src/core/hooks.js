...
export const GATE_EVENTS = new Set(['pre_tool', 'post_tool', 'session_directory', 'prompt_submit', 'agent_stop', 'before_branch', 'transform_llm_output']);
...
export class HookRunner {
  ...
  async fireGate(event, payload = {}) {
    ...
    //   {"text":"…"}   transform_llm_output only — reshapes the delivered
    //                  output; composes like args (later hooks see it).
    ...
    for (const h of entries) {
      ...
      try {
        const r = await this.#runEntry(h, { ...this.#payloadFor(event, payload), ...(rewritten !== undefined ? { args: rewritten } : {}), ...(transformed !== undefined ? { text: transformed } : {}) }, timeoutMs);
        ...
        // dedup-h #1922 — {"text":"…"} transform_llm_output only: the hook
        // reshapes the delivered LLM output text. Same composition rule as
        // args — a later entry's stdin sees the already-transformed text.
        // Content field: model-backed kinds need allowPromptInjection.
        if (structured && structured.text !== undefined) {
          if ((kind === 'prompt' || kind === 'agent') && !(h.allowPromptInjection === true || this.#allowInjection)) {
            this.audit?.write({ kind: 'HOOK_INJECTION_REFUSED', data: { event, form: kind, stripped: ['text'] } });
            return { deny: `${event} ${kind} hook attempted output transform without allowPromptInjection` };
          }
          if (typeof structured.text !== 'string' || !structured.text.trim()) {
            return { deny: `${event} hook returned a malformed text transform (expected a non-empty string)` };
          }
          transformed = structured.text.slice(0, MAX_OUTPUT_CHARS);
        }
      } catch (err) {
    ...
    if (rewritten !== undefined || transformed !== undefined || appends.length) {
      return { ...(rewritten !== undefined ? { args: rewritten } : {}), ...(transformed !== undefined ? { text: transformed } : {}), ...(appends.length ? { append: appends } : {}) };
    }
    return null;
  }
