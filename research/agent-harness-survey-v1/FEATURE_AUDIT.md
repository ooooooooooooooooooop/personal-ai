...
| G7 | **OS 级沙箱**（Seatbelt/Docker/容器 per-session） | Claude Code、Cursor、OpenHands Docker、Qwen serve、OpenClaw 后端注册表、mini-swe env swap、Hermes 容器后端、CodeArts 沙箱 | 🟡 sandbox provider 面有（wsl/docker/ssh argv 包装+拒绝未知后端）；#1291 已落 win32 kill-on-close 收容（detached watchdog：宿主 pid 死亡→taskkill /T 整树+命名容器 docker rm -f，孤儿窗口从"下次启动"缩到 ~500ms）；仍无资源限额/容器隔离 per-session | **P2**（Job Object 级 kill-on-close 已等价落地；WSL2/资源限额方向待裁） |
...
| G5 生命周期 hooks | 双层：workdir `.pai/hooks.json` 四事件**观察面**（agent 可写→永不许 veto，pre_tool 载入即拒）+ 实例私有 `<instance>/hooks.json` **gate 面**（agent 够不到→`pre_tool` 真否决：exit≠0 或 `{"deny"}` JSON→block，`match` 工具前缀过滤，热读，fail-closed）。**+#1143**：四形态条目（command/http/prompt/agent）装载期校验——未知 `type` 或无已知形态的条目在 gate 文件载入即抛、在 agent 可达文件审计丢弃（不静默死钩）；`allowPromptInjection` 闸——模型态钩输出默认只能否决（deny/block/requireApproval/reason），`text`/`context` 等内容注入字段须条目级或配置级显式开启，未开启时剥除+审计 `HOOK_INJECTION_REFUSED`、纯注入应答判失败（fail-closed）。**+#1188**：`message_sent` 出站观测事件（assistant message_end → {text≤4K, chars, model, usage} enriched payload）；`message:transcribed` 属语音输入面=边界声明 | **已有** | `host/src/core/hooks.js` fireGate/fireValue/`#load` 校验、`pi/src/bootstrap/decide.js` preToolGate |
...
### 28.64 648 清单逐条核销 #36：dedup-h #347 legacy `transport:"sse"`（2026-09-23）

- **判定**：IMPLEMENTED。2024-11-05 旧式 SSE 传输（Streamable HTTP 之前的双通道协议）。
...
### 28.70 648 清单逐条核销 #42：dedup-h #394 远程 MCP 10s 连接预算（2026-09-23）
...
### 28.72 648 清单逐条核销 #44：dedup-h #398 不可达 SSE 服务器连接预算（2026-09-23）
...
### 28.88 648 清单逐条核销 #62：dedup-h #697 masked credential prompt（2026-09-24）
...
### 28.109 648 清单逐条核销 #113：dedup-h #1546 memory-context——compaction: agent.compaction_model 指定摘要模型（2026-09-24）

- **行**：`dedup-h  1546  memory-context  compaction: agent.compaction_model指定摘要模型`。
- **判定**：**IMPLEMENTED**。基线核查：pi 原生压缩默认用**会话模型**（`_runDefaultCompaction(preparation, this.model, …)`），无 per-agent 摘要模型通道；但 `session_before_compact` 扩展事件收 `preparation`（`messagesToSummarize`/`firstKeptEntryId`/`tokensBefore`/`previousSummary`/`fileOps`）并可返回 `{compaction}` 接管摘要——落地缝天然存在。三层落地：
  - **profile 字段**：`.pai/agents/*.md` frontmatter `compaction_model: provider/model`（或裸 model=会话 provider）在 `envCapable` 信任门下解析——与 `model`/`effort` 同门，repo 种植的 profile 不能操纵子体摘要花费；解析进 `p.compactionModel`。
  - **委派通道**：`delegate.js` fail-closed 校验（非 pai-channel 目标不读 `PAI_COMPACTION_MODEL` → pre-spawn 拒绝 `unenforceable_compaction_model`，与 tools_deny/mcp_deny 同型）；`--compaction-model-b64` 专用桥旗标（`--env-json` 拒 `PAI_*`，b64 保任意拼写字节级透传）→ `PAI_COMPACTION_MODEL` env。
  - **执行缝**：`loopGovernanceExtension` 接 `compactionSummarize` 依赖——bootstrap 解析 `PAI_COMPACTION_MODEL`（env 章优先）→ 否则 `feature-models.json` `compaction` 条（operator 级等效面）→ 都无则不接管（返回 null，pi 原生摘要跑会话模型，零接管零风险）。配置存在时 handler 序列化 `messagesToSummarize`（untrusted 包裹 + customInstructions + previousSummary 合并）→ `judgeCall` 变体（2048 tokens/30s/事件 signal 贯通/budget-egress 同门）→ 返回 `{compaction: {summary, firstKeptEntryId, tokensBefore, details: {compactionModel, readFiles, modifiedFiles}}}`（details 镜像 pi `computeFileLists` 契约 + 模型标记）。**摘要器缺席/失败 → 返回 undefined 让原生摘要跑，`COMPACTION_MODEL_FALLBACK` 审计留痕，绝不静默**；成功记 `COMPACTION_MODEL` 审计。
- **证据**：`pi/tests/compaction-model.test.js` +5——(a) loop 单元：序列化 transcript 进 prompt、customInstructions+previousSummary 入 system、返回 `{compaction}` 全字段（details.readFiles 排除已改文件）、`COMPACTION_MODEL` 审计；(b) null/失败回调 → undefined + `COMPACTION_MODEL_FALLBACK`；(c) e2e：feature-models `compaction` 路由摘要到 `compact-x`（本地假端点收 model 断言 + transcript 内容断言 + 审计）；(d) `PAI_COMPACTION_MODEL` env 章覆盖同 provider 的 file 条（agent 级胜 operator 级）；(e) 无配置 → `res.compaction` undefined 不接管。`agentprofiles.test.js` +2——信任门解析（untrusted 剥/operator+trusted 保留）；`--compaction-model-b64` 旗标上桥 + b64 解码回原文 + 非 enforceable 目标 `unenforceable_compaction_model` 拒。loop-governance/m8-wiring/jobs-executor/wand 109/109 回归绿。
- **核销**：candidates-open #1546 → `candidates-resolved.tsv` #113。

### 28.110 648 清单逐条核销 #114：dedup-h #1590 models-routing——proxy: OpenAI-compatible 可设 Host header+legacy API（2026-09-24）

- **行**：`dedup-h  1590  models-routing  proxy: OpenAI-compatible可设Host header+legacy API`（源行描述段为另一 changelog 的错位截断片段——"Skills: compact skill file <location> paths…prompt seria"，源文件内即断尾；按标题语义为主判定，错位片段单列评估）。
- **判定**：**IMPLEMENTED（Host header）+ ALREADY_COVERED（legacy API）+ BOUNDARY（错位描述段）**。三分：
  - **Host header（IMPLEMENTED）**：声明面早已齐——`providers.X.headers` / `models[].headers` / auth.json headers 均 schema 合法且经 `withConfiguredAuth`/`model.headers` 合入请求头（pi 侧既有）。**真缺口在传输层**：undici fetch 按 Fetch 禁改头规范**静默剥 `host`**——虚拟主机路由网关（v-host 分发 OpenAI 兼容端点）声明的 `host` 头永远上不了线。本次在 `budgetfetch.js` 落地 `hostPreservingFetch`：gated fetch 检出请求携带 `host` 头时改走 node:http/https 传输（原值 verbatim 上线、CONNECT 目标仍由 URL 决定 → egress/budget 门禁继续审判真实目的地；content-length 自动补齐、AbortSignal 贯通、rawHeaders 逐对进 Response、可缓冲 body 直写/ReadableStream 管道）。未声明 `host` 的请求走原 undici 路径零变化；重定向不跟随（原样返回诚实暴露）。**位点=生成器源码**（gated fetch 是全部 provider 调用的唯一传输汇聚点）。
  - **legacy API（ALREADY_COVERED）**：`openai-completions`（legacy chat/completions）与 `openai-responses` 双 API 面早已支持——provider/model 级 `api` 字段 schema 合法（`ProviderConfigSchema.api`/`ModelDefinitionSchema.api`）、运行时 `Api` 联合类型含全部 openai-* 变体、我方 channel `api: spec.api`/`rtProv.api` 透传无损。
